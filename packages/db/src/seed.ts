...
async function main() {
  ...
  await reset(db, [publicSchema.subscribersToBatches]);
  // await reset(subscribersDb, subscribersSchema);
  // await reset(collectorsDb, collectorsSchema);
  ...
  const subscribers = await subscribersDb.query.users.findMany();
